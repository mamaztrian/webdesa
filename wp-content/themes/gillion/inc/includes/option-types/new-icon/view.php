	...
	<div class="js-option-type-new-icon-container">
		...
		<div class="fw-backend-option-fixed-width">
			<select class="js-option-type-new-icon-dropdown">
				<?php
					echo wp_kses( fw_html_tag('option', array('value' => 'all'), htmlspecialchars(esc_html__('All Categories', 'gillion'))), gillion_allowed_html_form() );
					foreach ($groups as $group_id => $group_title) {
						$selected = (isset($set['icons'][$data['value']]['group']) && $set['icons'][$data['value']]['group'] === $group_id);
						echo wp_kses( fw_html_tag('option', array('value' => esc_attr( $group_id ), 'selected' => esc_attr( $selected) ), esc_attr( htmlspecialchars($group_title))), gillion_allowed_html_form() );
					}
				?>
			</select>
			<div class="fw-icon-search-holder">
				<input type="text" class="fw-icon-search" placeholder="<?php esc_html_e( 'Search icon', 'gillion' ); ?>">
		...
		<div class="option-type-new-icon-list js-option-type-new-icon-list <?php echo esc_attr($set['container-class']) ?>">
			<?php
				foreach ($icons as $icon_id => $icon_tab) {
					...
					), true), gillion_allowed_html_icon_option() );
				}
			?>
		</div>
