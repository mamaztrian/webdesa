...
if( !class_exists('Widget_Recent_Posts') ) :
	class Widget_Recent_Posts extends WP_Widget {
		...
		function __construct() {
			$widget_ops = array( 'description' => esc_html__( 'Recent Posts', 'gillion' ) );

			parent::__construct( false, esc_html__( 'Shufflehound Recent Posts (outdated)', 'gillion' ), $widget_ops );
		}
		...
		function widget( $args, $instance ) {
			...
			$filepath = get_template_directory() . '/inc/widgets/posts-recent-old/views/widget.php';
		...
		function form( $instance ) {
			...
			$titles = array(
				'widget-title' => esc_html__( 'Recent Posts Title:', 'gillion' ),
				'items_per_page'     => esc_html__( 'Items Per Page:', 'gillion' ),
			);
